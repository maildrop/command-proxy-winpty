...
unsigned ( __stdcall pty_out_read_thread)( void *handle_value )
{
  ...
  if( handle != INVALID_HANDLE_VALUE ){
    ...
    while( ReadFile( handle , buffer->data() , DWORD(std::size( *buffer )) , &numberOfBytesRead , nullptr ) ){
      ...
      int const wnumber =
        MultiByteToWideChar( CP_UTF8 , 0 , buffer->data(), numberOfBytesRead , wide , sizeof(wide)/sizeof(wide[0]) - 1 );
      assert( wnumber < sizeof( wide ) / sizeof( wide[0] ) );
      wide[wnumber] = L'\0';
...
static HRESULT prepare_startup_information(HPCON hpc, STARTUPINFOEX& si)
{
    ...
    VERIFY( 0 == InitializeProcThreadAttributeList(NULL, 1, 0, &bytesRequired) );
    ...
    if( CreatePipe ( &pty_out[READ_SIDE], &pty_out[WRITE_SIDE], nullptr , 0 )){
      ...
      if( !FAILED( hr ) ){
        ...
        if( CreateProcess( NULL ,
          ...
          struct pump_handles handles = { GetStdHandle( STD_INPUT_HANDLE ) , pty_in[WRITE_SIDE] };
          
          uintptr_t stdinput_reader = 
            _beginthreadex( nullptr , 0 ,
                            []( void* handle_value)->unsigned{

                              const struct pump_handles * const handles =
                                reinterpret_cast<const struct pump_handles *>( handle_value );

                              if( INVALID_HANDLE_VALUE != handles->in ){

                                auto in_buffer = std::make_unique< std::array<char,4096> >();  // may be ACP
                                auto out_wide_buffer = std::make_unique< std::array<wchar_t,4096> >(); 
                                auto out_narrow_buffer = std::make_unique< std::array<wchar_t,4096 > >(); // may be UTF-8
                                ...
                                for(;;){
                                  ...
                                  if(! ReadFile( handles->in ,
                                                 reinterpret_cast<LPVOID>( in_buffer->data() ),
                                                 DWORD( std::size( *in_buffer ) ),
                                                 &numberOfBytesRead ,
                                                 nullptr )){
                                    DWORD const lastError = GetLastError();
                                    if( ERROR_OPERATION_ABORTED == lastError ){
                                      // Input Operation was canceled.
                                      OutputDebugString(TEXT(" stdinput_reader cancel at ReadFile()\n" ));
                                    }
                                    break;
                                  }

                                  {
                                    ...
                                    if( 0 < (number_of_converted_wchar =
                                      ...
                                      if( 0 < require_size ){
                                        ...
                                        if( static_cast<bool>(narrow) ){
                                          VERIFY( require_size == WideCharToMultiByte( CP_UTF8 , 0 ,
                                                                                       outbuffer , number_of_converted_wchar ,
                                                                                       narrow.get() , require_size ,
                                                                                       nullptr , nullptr ) );
                                          DWORD numberOfBytesWrite = 0;
                                          if( !WriteFile( handles->out ,
                                                          reinterpret_cast<LPVOID>(narrow.get()) , require_size ,
                                                          &numberOfBytesWrite , nullptr ) ) {
                                            DWORD lastError = GetLastError();
                                            if( ERROR_OPERATION_ABORTED == lastError ){
                                              // Input Operation was canceled.
                                              OutputDebugString(TEXT(" stdinput_reader cancel at WriteFile()\n" ));
                                            }
                                            break;
                                          }
                                          if(! FlushFileBuffers( handles->out ) ){
                                            ...
                                            if( ERROR_OPERATION_ABORTED == lastError ){
                                              // Input Operation was canceled.
                                              OutputDebugString(TEXT(" stdinput_reader cancel at FlushFileBuffers()\n" ));
                                            }
                                            break;
                                          }
                                        }
                                ...
                                SleepEx( 0 , TRUE ); /* consume User-APC  */
                              }
                              return EXIT_SUCCESS;
