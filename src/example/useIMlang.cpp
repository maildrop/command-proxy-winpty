...
static SHORT entry( int argc , char** argv)
{
  ...
  if( S_OK == (hr = CoCreateInstance(CLSID_CMultiLanguage, nullptr , CLSCTX_INPROC_SERVER, IID_PPV_ARGS(imlang3.GetAddressOf() ) )) ){
    std::cout << "OK" << std::endl;
  }

  DWORD mode = 0;
  char src[] = u8"こんにちは";
  UINT src_size = (UINT)(-1);
  UINT dst_size = 3;
  std::unique_ptr< BYTE[] > dst{ new BYTE[ dst_size ] };
  if( S_OK == imlang3->ConvertString( &mode , 65001 , 932 ,
                                      reinterpret_cast<BYTE*>(src),
                                      &src_size,
                                      dst.get(),
                                      &dst_size ) ){
